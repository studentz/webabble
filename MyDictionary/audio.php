<?php  
	...
	$sql= "SELECT `Audio`,`AudioMime`,`AudioTime` FROM  `CondenseTable` WHERE`ChineseWordId` = '$ChineseWordId' "; 
	...
	if($result = $myconn->query($sql)){
		...
		$row = $result->fetch_array(MYSQLI_ASSOC);
				...
				if (preg_match("/apache/i", PHP_SAPI)) {
					...
					(strtotime($ar['If-Modified-Since']) >= $AudioTime)) // and grater than image_time
					$send_304 = true;                                     
				 }

				 if ($send_304) {
					// Sending 304 response to browser "Browser, your cached version of image is OK  we're not sending anything new to you"
					header('Last-Modified: '.gmdate('D, d M Y H:i:s', $AudioTime).' GMT', true, 304);
					exit(); 
				  }
				// outputing Last-Modified header
				header('Last-Modified: '.gmdate('D, d M Y H:i:s', $AudioTime).' GMT',true, 200);
				// Set expiration time +1 year We do not have any photo re-uploading so, browser may cache this photo for quite a long time
				header('Expires: '.gmdate('D, d M Y H:i:s',  $AudioTime + 86400*365).' GMT',true, 200);
